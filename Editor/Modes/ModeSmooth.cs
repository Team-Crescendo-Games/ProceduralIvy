...
namespace TeamCrescendo.ProceduralIvy
{
    public class ModeSmooth : AMode
    {
            ...
            float smoothInsensity)
        {
            ...
            if (!smoothing) SelectBranchPointSS(currentEvent.mousePosition, brushSize);
            ...
            if (overBranch != null)
            {
                ...
                if (!currentEvent.alt && currentEvent.type == EventType.MouseUp && smoothing) StopSmoothing();

                //si no estamos en el forbiddenrect, y no estamos pulsando alt y es el ratón pcipal del ratón
                if (!forbiddenRect.Contains(currentEvent.mousePosition) && !currentEvent.alt &&
