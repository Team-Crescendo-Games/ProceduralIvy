...
namespace TeamCrescendo.ProceduralIvy
{
    public class UIZone_GeneralSettings : UIDropDownZone
    {
        ...
        {
            areaHeight = 265f;
            dropDownButton.timer = 1f;
            dropDownButton.unfolded = true;
        }

        public override void DrawZone(string sectionName, float areaMaxHeight, RealIvyWindow realIvyProWindow,
            ...
            Color bgColor, AnimationCurve animationCurve)
        {
            ...
            UIUtils.CustomObjectField(new Rect(0f, areaYSpace, areaRect.width / 2f, 25f),
                controller.selectedPreset, windowSkin, realIvyProWindow.oldSkin, RealIvyWindow.presetTex, 489136168,
                string.Empty,
            ...
            ivyParametersGUI.generateBranches = GUI.Toggle(new Rect(0f, areaYSpace, 133f, 20f),
                ivyParametersGUI.generateBranches, "Vines", windowSkin.toggle);
            ivyParametersGUI.generateLeaves = GUI.Toggle(new Rect(generalArea.width / 3f + 10f, areaYSpace, 133f, 20f),
                ivyParametersGUI.generateLeaves, "Leaves", windowSkin.toggle);
            ivyParametersGUI.generateLightmapUVs =
                GUI.Toggle(new Rect(generalArea.width / 3f * 2f + 10f, areaYSpace, 133f, 20f),
                    ivyParametersGUI.generateLightmapUVs, "Lightmap UVs", windowSkin.toggle);
            areaYSpace += 20f;

            areaYSpace += 10f;
            GUI.Label(new Rect(5f, areaYSpace, generalArea.width / 3f - 5f, 20f), "Vines material:", windowSkin.label);
            GUI.Label(new Rect(generalArea.width / 3f + 10f, areaYSpace, generalArea.width / 3f - 5f, 20f),
                "Leaves prefabs:", windowSkin.label);
            areaYSpace += 20f;

            ivyParametersGUI.branchesMaterial = UIUtils.CustomObjectField(
                new Rect(0f, areaYSpace, generalArea.width / 3f - 5f, 25f),
                ivyParametersGUI.branchesMaterial, windowSkin, realIvyProWindow.oldSkin, RealIvyWindow.materialTex,
            ...
            GUI.Label(new Rect(5f, areaYSpace + 35f, generalArea.width / 3f - 5f, 20f), "Layer Mask:",
                windowSkin.label);
            ivyParametersGUI.layerMask = EditorGUI.MaskField(
                new Rect(0f, areaYSpace + 55f, generalArea.width / 3f - 5f, 25f),
                InternalEditorUtility.LayerMaskToConcatenatedLayersMask(ivyParametersGUI.layerMask),
                InternalEditorUtility.layers,
                windowSkin.GetStyle("dropdown"));
            GUI.DrawTexture(new Rect(generalArea.width / 3f - 30f, areaYSpace + 55f, 25f, 25f),
                RealIvyWindow.downArrowTex);
            ivyParametersGUI.layerMask =
                InternalEditorUtility.ConcatenatedLayersMaskToLayerMask(ivyParametersGUI.layerMask);

            ivyParametersGUI.buffer32Bits = GUI.Toggle(new Rect(0f, areaYSpace + 100f, 133f, 20f),
                ivyParametersGUI.buffer32Bits, "32 bits mesh buffer", windowSkin.toggle);

            GUI.Box(new Rect(generalArea.width / 3f + 5f, areaYSpace, (generalArea.width / 3f - 5f) * 2f, 120f), "",
                windowSkin.GetStyle("list"));
            ...
            realIvyProWindow.leavesPrefabsScrollView = GUI.BeginScrollView(
                new Rect(generalArea.width / 3f + 5f, areaYSpace, (generalArea.width / 3f - 5f) * 2f, 100f),
                realIvyProWindow.leavesPrefabsScrollView,
                new Rect(0f, 0f, (generalArea.width / 3f - 21f) * 2f,
                    (ivyParametersGUI.leavesPrefabs.Count + 1) * 25f));

            for (var i = 0; i < ivyParametersGUI.leavesPrefabs.Count + 1; i++)
                if (i < ivyParametersGUI.leavesPrefabs.Count)
                {
                    if (ivyParametersGUI.leavesPrefabs[i] != null)
                    {
                        if (GUI.Button(new Rect(0f, i * 25f, (generalArea.width / 3f - leaveButtonsMargin) * 2f, 25f),
                                ivyParametersGUI.leavesPrefabs[i].name, windowSkin.GetStyle("listelement")))
                        ...
                        ivyParametersGUI.leavesProb[i] = EditorGUI.FloatField(
                            new Rect(generalArea.width / 3f * 2f - leaveButtonsMargin * 2f + 10, i * 25f + 2, 25f, 20f),
                            ivyParametersGUI.leavesProb[i], windowSkin.textArea);
                        GUI.Label(
                            new Rect(generalArea.width / 3f * 2f - leaveButtonsMargin * 2f - 8f, i * 25f + 2, 25f, 20f),
                            "%", windowSkin.GetStyle("whitelabel"));

                        if (GUI.Button(new Rect(generalArea.width / 3f * 2f - leaveButtonsMargin, i * 25f, 25f, 25f),
                                "", windowSkin.GetStyle("removeelement"))) RemoveLeavePrefab(ivyParametersGUI, i);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    UIUtils.DragAndDropArea(new Rect(0f, i * 25f, (generalArea.width / 3f - 5f) * 2f, 25f),
                        OnObjectDragged, realIvyProWindow.SaveParameters);
                }
        ...
        private void RemoveLeavePrefab(IvyParametersGUI ivyParametersGUI, int index)
        {
            ...
            for (var b = 0; b < RealIvyWindow.controller.infoPool.ivyContainer.branches.Count; b++)
            for (var l = 0; l < RealIvyWindow.controller.infoPool.ivyContainer.branches[b].GetNumLeaves(); l++)
        ...
        {
            var prefab = (GameObject)objectDragged;
        ...
        {
            if (RealIvyWindow.controller.infoPool.ivyParameters != null) RealIvyWindow.controller.OverridePreset();
        }
        ...
        private void SavePresetAs()
        {
            ...
            if (path != "") RealIvyWindow.controller.SaveCurrentParametersAsNewPreset(path);
        }
    }
}